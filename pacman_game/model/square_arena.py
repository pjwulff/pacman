...
class SquareArena(Arena):
    ...
    def _generate_nodes(self, width, height):
        ...
        for i in range(width):
            for j in range(height):
                if i < (width - 1):
                    nodes[i][j].add_geoneighbour(nodes[i+1][j])
                if 0 < i:
                    nodes[i][j].add_geoneighbour(nodes[i-1][j])
                if j < (height - 1):
                    nodes[i][j].add_geoneighbour(nodes[i][j+1])
                if 0 < j:
                    nodes[i][j].add_geoneighbour(nodes[i][j-1])
        ...
        self._inky_start = (nodes[width-1][height-1], nodes[width-1][height-2])
        self._join(self._inky_start[0], self._inky_start[1])

        self._clyde_start = (nodes[0][height-1], nodes[0][height-2])
